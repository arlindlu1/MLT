...
export default async function BlogIndex({
  // 👇 searchParams is a Promise in Next 15 server components
  searchParams,
}: {
  searchParams: Promise<{ category?: string }>
}) {
  const { category } = await searchParams // 👈 await before using

  const posts = await sanityClient.fetch(
    category ? postsByCategoryQuery : postsQuery,
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {categories.map((c: any) => (
              <Chip key={c.slug} href={`/blog?category=${c.slug}`}>
                {c.title}
              </Chip>
            ))}
          </div>
